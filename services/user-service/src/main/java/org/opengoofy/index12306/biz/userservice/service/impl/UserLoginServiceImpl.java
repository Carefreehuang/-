...
public class UserLoginServiceImpl implements UserLoginService {
    ...
    public Boolean hasUsername(String username) {
        // 首先去布隆过滤器进行判断，如果不存在，一定不存在，返回true，证明没有被注册过（语义稍有歧义）
    ...
    public UserRegisterRespDTO register(UserRegisterReqDTO requestParam) {
        // 抽象责任链上下文，使用Map存储了所有的处理队列，通过标识获取对应的处理队列
        // 每个处理队列又存储了对应的一些处理者，通过责任链上下文，可以获取到对应的处理队列，然后依次执行处理队列中的处理者
        // 判断是不是信息漏了，或者用户名被重用了，或者身份证号被拉黑（用户注销次数太多）
